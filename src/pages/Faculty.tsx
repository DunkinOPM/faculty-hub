...
import { MoreHorizontal, PlusCircle, LayoutGrid, List, Mail, Phone, Building } from "lucide-react";
...
const FacultyPage = () => {
  ...
  const renderFacultyGrid = () => (
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {faculty.map((member) => (
        <Link to={`/faculty/${member.id}`} key={member.id} className="group">
          <Card className="group flex flex-col overflow-hidden rounded-2xl border-border/50 shadow-sm transition-all duration-300 hover:border-primary/30 hover:shadow-xl h-full">
              ...
              <CardFooter className="p-2">
                  <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={(e) => { e.preventDefault(); handleEdit(member); }}>Edit Profile</DropdownMenuItem>
                          <DropdownMenuItem onClick={(e) => { e.preventDefault(); handleDelete(member.id); }} className="text-destructive">Delete Profile</DropdownMenuItem>
                      </DropdownMenuContent>
                  </DropdownMenu>
              </CardFooter>
          </Card>
        ...
        <Table>
            ...
            {faculty.map((member) => (
                <TableRow key={member.id}>
                    <TableCell>
                      <Link to={`/faculty/${member.id}`} className="flex items-center gap-4">
